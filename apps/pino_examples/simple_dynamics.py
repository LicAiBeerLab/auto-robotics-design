...
vq, J_closed = inverseConstraintKinematicsSpeed(
    ...
    q,
    ee_id_g,
    robo.data.oMf[ee_id_g].action @ np.zeros(6),
)
...
ad_arr = np.zeros((N_it, len(robo.actuation_model.idvfree)))

prev_Jmot, prev_Jfree = jacobian_constraint(
    robo.model,
    robo.data,
    robo.constraint_models,
    robo.constraint_data,
...
for i in range(N_it):
    ...
    b = pin.rnea(robo.model, robo.data, q, vq, np.zeros(robo.model.nv))
    ...
    if i == 0:
        x_body_des = x_body_curr + np.array([0, 0, 0.1, 0, 0, 0])
    ...
    a_d = -np.linalg.pinv(Jfree) @ (
        d_Jmot @ vq[robo.actuation_model.idvmot]
        + d_Jfree @ vq[robo.actuation_model.idvfree]
    )
    ...
    g = pin.computeGeneralizedGravity(robo.model, robo.data, q)
    C = pin.computeCoriolisMatrix(robo.model, robo.data, q, vq)
    g_a = Jda.T @ E_tau.T @ g
    ...
    # tau_a = Ma @ (ddq_des_traj[i] + K * (qa_d - q_a) + Kd * (vqa_d - vq_a)) + g_a + C_a @ vq_a #+ Jda.T @ E_tau.T @ M @ E_tau @ np.concatenate((np.zeros(nvmot), a_d))
    # tau_a = Ma @ a_a - b_a
    # tauq = ctrl.compute(q, vq, qa_d, vqa_d, ddq_des_traj[i])
    tauq = ctrl.compute(q, vq, x_body_des, np.zeros(6))
    # cho = (
    #     J_closed.T
    #     @ (Kimp @ (x_body_des - x_body_curr) + Kdimp @ (np.zeros(6) - v_body))
    #     + g_a
    #     # + C_a @ vq_a
    #     # + Jda.T @ E_tau.T @ M @ E_tau @ np.concatenate((np.zeros(nvmot), a_d))
    # )
...
# name_x = ["x", "y", "z"]
# for i, name in enumerate(name_x):
#     ax[i].plot(t_arr, x_body_des_arr[:, i], label="des", linestyle="--", linewidth=2)
#     ax[i].plot(t_arr, x_body_arr[:, i], label="curr")
#     ax[i].set_xlabel("time, s")
#     ax[i].set_ylabel(name + ", m")
#     ax[i].set_xlim((t_arr[0], t_arr[-1]))
#     ax[i].grid()
