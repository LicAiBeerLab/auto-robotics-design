...
def confirm_topology():
    ...
    st.slider_constants = st.session_state.jp_positions.copy()
...
def slider_change():
    st.session_state.slider_constants = st.session_state.jp_positions.copy()
...
Если для сочленения нет слайдеров, то данное сочленение в соответствующей структуре является неподвижным.""")
    ...
    with st.sidebar:
        ...
        jp_label = st.radio(label='Сочленение', options=labels.values(), index=0, key='joint_point_choice', horizontal = True,on_change=slider_change)
...
if st.session_state.stage == 'workspace_visualization':
    ...
    with st.sidebar:
        ...
        if trajectory_type == "шаг":
            start_x = st.slider(
                label="начало_x", min_value=-0.3, max_value=0.3, value=-0.14)
            start_z = st.slider(
                label="начало_z", min_value=-0.4, max_value=-0.2, value=-0.34)
            height = st.slider(
                label="высота", min_value=0.02, max_value=0.3, value=0.1)
