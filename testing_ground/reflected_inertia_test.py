...
for pair_j_act in pairs:
    ...
    robo, free_robo = jps_graph2pinocchio_robot(graph, builder)
    q = pin.neutral(robo.model)
    q = closedLoopProximalMount(
        robo.model, robo.data, robo.constraint_models, robo.constraint_data, q
    )
    ...
    ids = np.array(
        [robo.model.joints[robo.model.getJointId(name)].idx_v for name in nominal_names]
    )
    E = np.zeros_like(M)
    ...
    print(f"Joints Order: {nominal_names}")
    print(
        f"Reflected Inertia of Acts: {[(j_a[0], np.round(j_a[1].reduction_ratio ** -2 * j_a[1].inertia,5)) for j_a in pair_j_act]}"
    )
    print(
        "Difference Inertia Matrix in initial configuration with armature and without"
    )
