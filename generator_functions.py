...
def build_main_branch(n_link_options:Tuple[float] = (2,3,4), length_range:Tuple[float] = (0.2, 1.4), angle_range:Tuple[float] = (-pi/3, pi/3), ender_fixed:bool = True):
    body_counter = 0
    joint_counter = 0
    ...
    for i in range(main_branch_links-1):
        ...
        main_branch.append(JointPoint(r=new_coordenates, w=np.array([0, 1, 0]),  name=f"J{i}m"))
        joint_counter+=1
        body_counter+=1
...
def find_connect_point(first_point: np.array, second_point: np.array, length_factor: float = 0.8, width_factor: float = 0.5) -> np.array:
    ...
    total_shift = direction * sample_length_shift + ort_direction / \
        np.linalg.norm(ort_direction)*(length_factor -
                                       abs(sample_length_shift))*sample_width_shift * half_length
...
def sample_secondary_branch(graph, main_branch, length_range:Tuple[float] = (0.3, 0.6), dof_reduction:int = 0,branch_id=0):
    ...
    if idx_sample != 0:
        ...
        new_joint = JointPoint(r=pos, w=np.array(
            [0, 1, 0]), name=f"J0_{branch_id}")
        local_joints+=1
        secondary_branch = [[main_branch[idx_sample-1], main_branch[idx_sample]], new_joint]
    else:
        # idx == 0 means attached to ground
        pos = np.array([np.random.uniform(-1, 1), 0, 0])
        new_joint = JointPoint(r=pos, w=np.array(
            [0, 1, 0]), attach_ground=True, name=f"J0_{branch_id}")
        local_joints+=1
    ...
    while not attach > 0:
        ...
        if attach:
            ...
            if current_joint.attached != -1:
                ...
                if len(new_set) == 0: return False # there is no way to connect the branch
            ...
            if idx_sample != 0:
                ...
                new_joint = JointPoint(r=pos, w=np.array([0, 1, 0]), attach_endeffector=False,  name=f"J{i}_{branch_id}")
                local_joints+=1
                local_bodies+=1
                secondary_branch+=[new_joint,[main_branch[idx_sample-1], main_branch[idx_sample]]]
            else:
                pos = np.array([np.random.uniform(-1, 1), 0, 0])
                new_joint = JointPoint(r=pos, w=np.array(
                    [0, 1, 0]), active=False, attach_ground=True, name=f"J{i}_{branch_id}")
                secondary_branch.append(new_joint)
                local_joints+=1
                local_bodies+=1
            new_joint.attached = idx_sample
            ...
            new_joint = JointPoint(r=new_pos, w=np.array([0, 1, 0]), active= False, attach_endeffector=False,  name=f"J{i}_{branch_id}")
    ...
    if delta_dof<dof_reduction: return False
    ...
    if delta_dof - dof_reduction > min(len(triangle_idx), len(idx_set)): return False
    ...
    while delta_dof>dof_reduction:
        ...
        new_joint_triangle = JointPoint(r=new_pos, w=np.array([0, 1, 0]), active= False, attach_endeffector=False,  name=f"J{i}_{j}_{branch_id}")
        i+=1
        if idx_sample != 0:
            first_joint = main_branch[idx_sample-1].r
            second_joint = main_branch[idx_sample].r
            new_pos = find_connect_point(first_joint, second_joint)
            new_joint = JointPoint(r=new_pos, w=np.array([0, 1, 0]), active= False, attach_endeffector=False,  name=f"J{i}_{j}_{branch_id}")
            new_branch = [[triangle_list[triangle_sample-1], triangle_list[triangle_sample]],new_joint_triangle, new_joint, [main_branch[idx_sample-1], main_branch[idx_sample]]]
        else:
            pos = np.array([np.random.uniform(-1, 1), 0, 0])
            new_joint = JointPoint(r=pos, w=np.array([0, 1, 0]), active=False, attach_ground=True, name=f"J{i}_{branch_id}")
            new_branch = [[triangle_list[triangle_sample-1], triangle_list[triangle_sample]], new_joint_triangle, new_joint]
        local_bodies+=1
        local_joints+=2
...
def generate_graph():
    ...
    while dof>2 or zero_reduction:
        ...
        i = 0
        while not sample_secondary_branch(graph, main_branch, branch_id=b_idx,dof_reduction=-sample_dof_reduction):
