...
class TwoLinkGenerator():
    ...
    def build_standard_two_linker(self, knee_pos:float = -0.5,nominal_length = 1, right_shift = np.tan(np.pi/6)/2):
        ...
        constrain_dict[knee_joint.name] = {'optim':False, 'x_range':(-0.2, 0.2), 'z_range':(-0.2, 0.2)}
    ...
    def add_tl_branch(self,graph, graph_dict, constrain_dict,inner:bool = True, shift = 0.25, ground:bool = True):
        ...
        branch_knee_joint = JointPoint(r=branch_knee_pos,
            w=np.array([0, 1, 0]),
            name="TL_branch_knee")
        constrain_dict[branch_knee_joint.name] = {'optim':True, 'x_range':(-0.5, 0.5), 'z_range':(-0.5, 0.5)}
        branch_connection = JointPoint(r=connection_point,
            w=np.array([0, 1, 0]),
            name="TL_branch_connection")
        constrain_dict[branch_connection.name] = {'optim':True, 'x_range':(-0.2, 0.2), 'z_range':(-0.2, 0.2)}
        ...
        if ground:
            ...
            constrain_dict[branch_ground_joint.name] = {'optim':True, 'x_range':(-0.4, 0.4)}
            ...
            branch_ground_joint = JointPoint(r=ground_connection,
                ...
                active=True,
                name="TL_branch_ground")
            constrain_dict[branch_ground_joint.name] = {'optim':True, 'x_range':(-0.2, 0.2), 'z_range':(-0.2, 0.2)}
    ...
    def add_fl_branch(self, graph, graph_dict, constrain_dict, inner:bool = True, shift = 0.5, ground:bool = True, variant = 0):
        ...
        top_joint = JointPoint(r=connection_top,
            w=np.array([0, 1, 0]),
            name="FL_branch_top")
        constrain_dict[top_joint.name] = {'optim':True, 'x_range':(-0.2, 0.2), 'z_range':(-0.2, 0.2)}
        bot_joint = JointPoint(r=connection_bot,
            w=np.array([0, 1, 0]),
            name="FL_branch_bot")
        constrain_dict[bot_joint.name] = {'optim':True, 'x_range':(-0.2, 0.2), 'z_range':(-0.2, 0.2)}
        ...
        if variant == 0:
            ...
            w=np.array([0, 1, 0]),
            name="FL_branch_knee")
            constrain_dict[j1.name] = {'optim':True, 'x_range':(-0.4, 0.4), 'z_range':(-0.4, 0.4)}
            j2 = JointPoint(r=pos_2,
            w=np.array([0, 1, 0]),
            name="FL_branch_hip")
            constrain_dict[j2.name] = {'optim':True, 'x_range':(-0.4, 0.4), 'z_range':(-0.4, 0.4)}
            add_branch(graph,[[ee,knee_joint], bot_joint, j1, j2, branch_ground_joint])
            j3 = JointPoint(r=(pos_1+pos_2)/2,
            w=np.array([0, 1, 0]),
            name="FL_branch_additional")
            constrain_dict[j3.name] = {'optim':True, 'x_range':(-0.4, 0.4), 'z_range':(-0.4, 0.4)}
            ...
            for i, pos in enumerate(new_joint_pos):
                ...
                constrain_dict[joint.name] = {'optim':True, 'x_range':(-0.4, 0.4), 'z_range':(-0.4, 0.4)}
                branch.append(joint)
    ...
    def get_standard_set(self, knee_pos=-0.5, shift=0.5):
        ...
        for inner in [True, False]:
            for ground in [True, False]:
                graph, graph_dict, constrain_dict= self.build_standard_two_linker(knee_pos=knee_pos)
                graph, constrain_dict = self.add_tl_branch(graph, graph_dict,constrain_dict, inner=inner, ground=ground,shift=shift)
                answer_list.append((graph, constrain_dict))
            for i in self.variants:
                graph, graph_dict, constrain_dict = self.build_standard_two_linker(knee_pos=knee_pos)
                graph, constrain_dict =self.add_fl_branch(graph, graph_dict, constrain_dict,inner=inner,variant=i,shift = shift)
                answer_list.append((graph, constrain_dict))
        return answer_list
