...
def draw_joint_point_widjet(graph: nx.Graph, labels=0, draw_legend=True, draw_lines=False, patches_list=[]):
    ...
    plt.plot(G_pos[:,0], G_pos[:,1], "o", label="Body joint", color="silver", ms=10)
    plt.axis("equal")
    ...
    # plt.axis('on')
    if EE_pos.size != 0:
        plt.plot(EE_pos[:,0], EE_pos[:,1], "o", label="End-effector", ms=10, color="lightsteelblue")
    plt.plot(active_j_pos[:,0], active_j_pos[:,1], "og",
             markersize=22, 
             fillstyle="none", label="Actuated")
    if draw_legend: plt.legend()
